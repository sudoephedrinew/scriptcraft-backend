...
export default async function handler(request, response) {
  ...
  // 2. Authorize the request from your Android app
  const appApiKey = process.env.APP_API_KEY; // Your secret key stored in Vercel
  const clientApiKey = request.headers['x-api-key']; // The key sent from your app

  // First, check if the server even has the key configured.
  if (!appApiKey) {
    return response.status(500).json({ message: 'Application API key not configured on the server.' });
  }

  // Now, check if the client's key matches the server's key.
  if (clientApiKey !== appApiKey) {
    return response.status(401).json({ message: 'Unauthorized: Invalid API Key' });
  }

  // 3. Retrieve the secret Google Gemini API key from Vercel's Environment Variables
  const geminiApiKey = process.env.GEMINI_API_KEY;
  if (!geminiApiKey) {
    return response.status(500).json({ message: 'Gemini API key not configured on the server.' });
  }

  try {
    // 4. Get the raw inputs from the app's request body
    ...
    // 5. Define the System Instruction and Prompt Template on the SERVER
    ...
    // 6. Construct the request to the actual Google Gemini API
    ...
    // 8. Extract the generated text and send it back to the Android app
